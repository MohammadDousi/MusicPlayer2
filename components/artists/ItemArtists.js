      ...
      {data &&
        data.map((items) => (
          // <Link key={items.id} href={`/track/${items.id}`}>
            <div key={items.id} className="w-[110px] h-[155px] md:w-auto md:h-auto p-2 flex flex-col justify-center items-center gap-3 bg-slate-700/50 hover:bg-slate-700 duration-300 rounded-2xl">
              <Image
