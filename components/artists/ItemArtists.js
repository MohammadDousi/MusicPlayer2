      ...
      {data &&
        data.map((items) => (
          // <Link key={items.id} href={`/track/${items.id}`}>
            <div key={items.id} className="w-[11 0px] h-[155px] p-2 flex flex-col justify-center items-center gap-3 bg-slate-700/50 hover:bg-slate-700 duration-300 rounded-2xl">
              <Image
